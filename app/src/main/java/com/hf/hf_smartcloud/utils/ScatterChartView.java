...
//            canvas.drawCircle(x, y, dp7, linePaint);
//            canvas.drawLine(x, yOri,x,y,mPaint);
//
//        }
//    }
//
...
///**
// * 自定义折线图
// * Created by guoxuliang on 19/11/29.
// */
//public class ScatterChartView extends View {
//    //xy坐标轴颜色
//    private int xylinecolor = 0xffe2e2e2;
...
//            case MotionEvent.ACTION_DOWN:
//                startX = event.getX();
//                Log.i("startX--xInit","startX:"+startX);
//                break;
//            case MotionEvent.ACTION_MOVE:
//                if (interval * xValue.size() > width - xOri) { //当期的宽度不足以呈现全部数据
//                    float dis = event.getX() - startX;
...
//                    if (xInit + dis < minXInit) {
//                        xInit = minXInit;
//                        Log.i("startX--xInit1","xInit:"+xInit);
//                    } else if (xInit + dis > maxXInit) {
//                        xInit = maxXInit;
//                        Log.i("startX--xInit2","xInit:"+xInit);
//                    } else {
//                        xInit = xInit + dis;
//                        Log.i("startX--xInit3","xInit:"+xInit);
//                    }
//                    invalidate();
//                }
//                break;
//            case MotionEvent.ACTION_UP:
////                clickAction(event);
//                scrollAfterActionUp();//手指抬起后的滑动处理
//                this.getParent().requestDisallowInterceptTouchEvent(false);
...
//                }
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        Log.i("VIEW-dispatchTouchEvent","VIEW"+event.getAction());
...
//        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        obtainVelocityTracker(event);
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN://按下View（所有触摸事件的开始）
            ...
            case MotionEvent.ACTION_MOVE://滑动View
                if (interval * xValue.size() > width - xOri) {//当期的宽度不足以呈现全部数据
                    float dis = event.getRawX() - startX;
                    startX = event.getRawX();
                    if (xInit + dis < minXInit) {
                        xInit = minXInit;
                        invalidate();
            ...
            case MotionEvent.ACTION_UP://抬起view（与Down对应）
                Log.i("VIEW","VIEW"+event.getAction());
                clickAction(event);
                scrollAfterActionUp();
                this.getParent().requestDisallowInterceptTouchEvent(false);
                recycleVelocityTracker();
                break;
            case MotionEvent.ACTION_CANCEL://结束事件（非认为结束）
                this.getParent().requestDisallowInterceptTouchEvent(false);
                recycleVelocityTracker();
                break;
        }
