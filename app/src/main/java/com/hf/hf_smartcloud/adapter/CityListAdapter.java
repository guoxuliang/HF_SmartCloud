...
public class CityListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public CityListAdapter(List<String> datas, Context context, boolean hasMore) {
        // 初始化变量  
        this.datas = datas;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == normalType) {
            return new NormalHolder(LayoutInflater.from(context).inflate(R.layout.item_city_list, null));
        } else {
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder instanceof NormalHolder) {
            for (int i = 0; i < datas.size(); i++) {
                ((NormalHolder) holder).city_name.setText(datas.get(i));
            }
